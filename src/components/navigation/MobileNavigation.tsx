...
const MobileNavigation = ({ navigationItems, darkMode, styles }: NavigationProps) => {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const location = useLocation();

  const toggleMobileMenu = useCallback(() => setMobileMenuOpen(prev => !prev), []);

  const getNavButtonStyle = useCallback((itemPath: string) => {
  ...
  return (
    <>
      ...
      {mobileMenuOpen && (
        <div className={`md:hidden border-t ${styles.border} py-4 absolute top-16 left-0 right-0 ${styles.bg.secondary} z-50`}>
